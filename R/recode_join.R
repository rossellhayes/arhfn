...
recode_join <- function(.tbl, col, code_tbl, code_col = code) {
  ...
  code_tbl <- select(code_tbl, {{col}}, {{code_col}}) %>%
    rename_at(code_col_name, function(...) ".code")

  replacement <- data_tbl %>%
    rename_at(col_name, function(...) ".code") %>%
    left_join(code_tbl, by = ".code") %>%
    select({{col}})

  data_tbl[which(names(data_tbl) == col_name)] <- replacement
